...
/**
 * query -TicketMaster API - > jsonArray
 * 
 * 			
 * 
 * */
...
public class TicketMasterAPI {
	...
	private static final String API_KEY = "gMs4jVtW4qkEkQLDYhLxv9EYlw9aWdLL";
	...
	private String getAddress(JSONObject event) throws JSONException {
		if (!event.isNull(EMBEDDED)) {
			...
			//				System.out.println(address.getString(LINE1));
					...
					//		System.out.println(address.getString(LINE2));
						}
						if (!address.isNull(LINE3)) {
							sb.append('\n');
							sb.append(address.getString(LINE3));
					//		System.out.println(address.getString(LINE3));
						}
					}
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		// Encode keyword in url since it may contain special characters
			...
			try {
				//encode why ? cause url has some encoding convention like ' ' > 20%
				keyword= java.net.URLEncoder.encode(keyword, "UTF-8");
			} catch (Exception e) {
				e.printStackTrace();
			}
			
			// Convert lat/lon to geo hash  lat + long + accuracy
